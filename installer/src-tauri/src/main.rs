...
use custom_rust_dist::cli::CliOpt;
...
use xuanwu_installer::{cli, Result};
...
fn main() {
    tauri::Builder::default()
        .setup(|app| {
            ...
            if let Some(cli_opt) = cli::tauri_cli_to_cli_opt(&matches) {
                dbg!(&cli_opt);
                CLI_ARGS
                    .set(cli_opt)
                    .expect("failed to cache commandline args");
            }
            Ok(())
        })
