...
pub(crate) async fn install_components(
    ...
    // 在一个新线程中执行安装过程
    let install_thread =
        spawn_install_thread(window, components_list, install_dir, manifest, is_update).await;
    // 在主线程中接收进度并发送事件
    let gui_thread = spawn_gui_update_thread(window_clone, install_thread, msg_recvr).await;

    if gui_thread.is_finished() {
        gui_thread.await.expect("unable to join GUI thread")?;
    }
    Ok(())
}

async fn spawn_install_thread(
    ...
    tokio::spawn(async move {
        ...
        if is_update {
            config.update(components_list).await?;
        } else {
            config.install(components_list).await?;
        }
...
pub(crate) async fn spawn_gui_update_thread(
    win: Arc<tauri::Window>,
    install_handle: JoinHandle<anyhow::Result<()>>,
